...
.EXAMPLE
Get-Content .\secrets.txt | .\encryptor.ps1 -key "mysecret"
Encrypts each line of the file individually via pipeline streaming.

.EXAMPLE
Get-Content .\data.txt -Raw | .\encryptor.ps1 -key "mysecret" | Set-Content .\data-enc.txt
Encrypts an entire file as a single string. Using -Raw is much faster for files because
...
process {
    # If clipboard input mode, read from clipboard on first process call
    if ($clipboardInput) {
        $text = Get-Clipboard -Raw
        if ([string]::IsNullOrEmpty($text)) {
            Throw "Clipboard is empty."
        }
        $clipboardInput = $false  # only read once
    }
